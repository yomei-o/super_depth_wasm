...
**最初から最後まで遊べます。** Bio_100% のロゴ → タイトル →
ステージ 1〜12 → 12 面クリアでエンディング（そのまま 1 面へ続く）／
ゲームオーバー → ネーム入力 → ランキング → タイトル。
続きの作業に入る人は [RESUME.md](RESUME.md) の冒頭「引き継ぎ」から読んでください
（いまどこまで／次にやること／刺された罠）。

遊ぶ: https://yomei-o.github.io/super_depth_wasm/

できていること: **起動時の Bio_100% ロゴから始まって 4 種類の面が全部
遊べて、音も鳴ります。**
...
面クリアと死亡の遷移、フェード、ポーズ、BGM 15 曲と効果音 6 種（内蔵ビープ
1 音）。コマ数も原典どおり（VSYNC 5 回に 1 コマ＝毎秒 11）。
...
起動すると、まず **Bio_100% のロゴ**（`FUN_1000_dbb2`）が出ます。絵では
なく 13 個の四角形と 13 個の台形で、「Bio」が画面外から飛び込んで縮み、
その下に「100%」が現れて白から緑へ色が変わる —— GRCG のプレーンを 1 枚
ずつ使い分けた 1991 年らしい作りです（ジングルは曲 1）。

面と面のあいだには演出が 3 本（海から飛び立つ／大気圏を抜ける／警報）、
タイトルには `DEPTH.FNT` の外字で組んだロゴとメニューとクレジット、
`Record` には `DEPTH.SCR` のランキングが出ます。

ボスを倒すと `Destroyed!` とジングルが入り、**12 面を倒すとエンディング**に
なります（惑星の横を通り過ぎて `Congratulation!!`、敵 18 体を名前つきで出す
`Cast`、そしてクレジット）。作者の `DEPTH.DOC` が言うとおり、
そこでゲームが終わるわけではなく、得点を持ったまま 1 面から続きます。

**原典から読み出せた挙動はひととおり入っています。**

移植の都合で原典と違うのは 3 つだけで、どれも「移植先に同じものが無い」ことが
理由です。

| | 原典 | 移植 |
|---|---|---|
| `Exit`・タイトルの ESC/Q・ポーズ中の Q | DOS へ戻る | ネイティブはウィンドウを閉じる。ブラウザは戻る先が無いので何もしない |
| ランキング | 読んだのと同じ `DEPTH.SCR` に書き戻す | ネイティブは実行ファイルの隣に書く（配布物を書き換えないため）。ブラウザは `localStorage` |
| 音の始まり | 起動した瞬間から鳴る | ブラウザは操作があるまで音を出させてくれないので、最初のキーかクリックから |

書いたものは逆コンパイル出力と 1 行ずつ突き合わせ直してあります。
何をどこまで見たかは [RESUME.md](RESUME.md) の「突き合わせ済みの範囲」に
あります。作者同梱の `DEPTH.DOC` / `DEPTHBGM.DOC` とも突き合わせました
（アイテムの効果・上下移動の可否・曲名がそこで確定しました）。
...
— 3 つのビルド、全画面の描画、長時間走らせて止まらないこと、ポーズの 3 つの
道、12 面クリアのエンディングが最後まで流れること、ネイティブと WASM の
1 ピクセル比較、そして音（ネイティブの WAV、ページが読む `.wasm` が鳴ること、
タイトルで鳴っているのが THEME であること）。
...
node tests/wasm_audio.js                    # WASM 側から音が出ているか
python tools/songcmp.py a.wav b.wav         # 2 つの WAV が同じ曲か
...
![pause](docs/screen_pause.png)

12 面をクリアすると:

![ending](docs/screen_ending.png)
![cast](docs/screen_cast.png)
![credits](docs/screen_credits.png)
