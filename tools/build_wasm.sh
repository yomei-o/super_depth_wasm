...
# The data files are baked in with --embed-file so the loaders keep using
...
SRC="$SRC src/stage_space.c src/stage_boss.c src/title.c src/opening.c src/record.c src/cut.c src/name.c src/ending.c"
...
# emcc's exit status back, so without the check at the end a compile error is
# announced as a successful build and the stale .wasm stays in place - which
# then quietly passes the native-versus-WASM comparison.
mkdir -p tmp
...
if [ ! -f superdepth.wasm ] || [ ! superdepth.wasm -nt "$STAMP" ]; then
    echo "emcc did not rewrite superdepth.wasm - the build failed" >&2
    exit 1
fi
